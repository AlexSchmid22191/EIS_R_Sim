...
current_data['ex_current'] = ox_eq**2 * hole_eq**4 / fe_eq**4 * 1/(1+100*current_data['pressure'])

# Calculate net currents
current_data['current'] = current_data['ex_current'] * \
                          ((ox/ox_eq)**2*(hole/hole_eq)**4*(fe/fe_eq)**-4 - (vac/vac_eq)**2)
...
save('Current_Data_Model_5_Shifted.npy', current_data)
