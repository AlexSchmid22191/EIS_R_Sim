...
for i in (2300, 2200, 2100, 2000, 1900):
    ax_abs_uh.plot(data['overpotential'][1::50, i], abs(data['current'][1::50, i]), linestyle='-',
                   label='$10^{%d}$ bar' % log10(data['pressure'][1, i]))

# High oxygen partial pressures
for i in (1800, 1700, 1600, 1500, 1400):
    ax_abs_hi.plot(data['overpotential'][1::50, i], abs(data['current'][1::50, i]), linestyle='-',
                   label='$10^{%d}$ bar' % log10(data['pressure'][1, i]))

# Medium oxygen partial pressures
for i in (1300, 1200, 1100, 1000, 900):
    ax_abs_me.plot(data['overpotential'][1::50, i], abs(data['current'][1::50, i]), linestyle='-',
                   label='$10^{%d}$ bar' % log10(data['pressure'][1, i]))

# Low oxygen partial pressures
for i in (800, 700, 600, 500, 400):
    ax_abs_lo.plot(data['overpotential'][1::50, i], abs(data['current'][1::50, i]), linestyle='-',
                   label='$10^{%d}$ bar' % log10(data['pressure'][1, i]))
...
fig_abs_co, ax_abs_co = subplots(nrows=1, ncols=2, figsize=(5.75, 2.5))

# Highest oxygen partial pressures
for i in (2300, 2200, 2100, 2000, 1900):
    ax_abs_co[0].plot(data['overpotential'][::50, i], abs(data['current'][::50, i]),
                      label='$10^{%d}$ bar' % log10(data['pressure'][1, i]))
for i in (2300, 2200, 2100, 2000, 1900):
...
# High oxygen partial pressures
for i in (1800, 1700, 1600, 1500, 1400):
    ax_abs_co[1].plot(data['overpotential'][::50, i], abs(data['current'][::50, i]),
                      label='$10^{%d}$ bar' % log10(data['pressure'][1, i]))
for i in (1800, 1700, 1600, 1500, 1400):
...
ax_abs_co[0].set_title(r'(a) Very high $p_\mathrm{O_2}$', color='#006699')
ax_abs_co[1].set_title(r'(b) High $p_\mathrm{O_2}$', color='#006699')
