...
colmesh_p = p_ax.pcolormesh(data['pressure'], data['overpotential'], data['p_slope'], vmin=0.0, vmax=0.25)
cbar_p = p_fig.colorbar(colmesh_p, ax=p_ax, label=r'$\frac{d\ln j}{d\ln p}$', orientation='horizontal')
cbar_p.ax.minorticks_off()

colmesh_n = n_ax.pcolormesh(data['pressure'], data['overpotential'], data['n_slope']*k*T/e, vmin=-2, vmax=1)
