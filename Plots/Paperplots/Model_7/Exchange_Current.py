...
ax.yaxis.set_major_locator(LogLocator(numticks=19))
ax.yaxis.set_minor_locator(LogLocator(numticks=19, subs=range(1, 10)))
ax.yaxis.set_minor_formatter(NullFormatter())

for label in ax.yaxis.get_ticklabels()[::2]:
    label.set_visible(False)

slope_marker(origin=(1e3, 1e5), slope=0.5, size_frac=0.1, invert=True, ax=ax)
slope_marker(origin=(1e-12, 1e-4), slope=0.75, size_frac=0.1, invert=True, ax=ax)

fig.tight_layout()

fig.savefig('Plots/Exchange_Current_Model_7.pdf')
fig.savefig('Plots/Exchange_Current_Model_7.png')
fig.savefig('Plots/Exchange_Current_Model_7.tiff', dpi=300)
