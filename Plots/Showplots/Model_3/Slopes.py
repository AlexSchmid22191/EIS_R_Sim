...
data = load('../../../Currents_Resistances_Model_3/Slope_Data_Model_3.npy')
...
colmesh_p = axes[0].pcolormesh(data['pressure'], data['overpotential'], data['p_slope'], vmin=0, vmax=0.25)
cbar_p = fig.colorbar(colmesh_p, ax=axes[0], label=r'$\frac{d\ln j}{d\ln p}$')
cbar_p.ax.minorticks_off()

colmesh_n = axes[1].pcolormesh(data['pressure'], data['overpotential'], data['n_slope']*k*T/e, vmin=0, vmax=2)
